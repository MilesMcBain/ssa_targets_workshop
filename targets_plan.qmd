...
- Running `tar_invalidate(occurences)` means the next time the plan is run `occurrences` will be rebuilt, which MAY trigger downstream targets to be rebuilt, but not necissarily if targets identifies the value hasn't changed. The stored value and metadata is retained.
...
The graph of targets defined in the plan can be visualised with either
  - `tar_visnetwork()`
  - `tar_gimpse()` - doesn't show information about which targets are up to date
...
not dependent on eachother, so it can help you understand the scope for
...
The main elements are:
  ...
  1. `tar_load()` the targets you want to work with interactively from the targets store with `tar_load()`
    - There is an RStuido Addin that ships with targets that can load the target under the cursor from the store. It is highly recommended that you make a keyboard shortut for this. See [creating a keyboard shortcuts for RStudio addins](https://docs.posit.co/ide/user/ide/guide/productivity/add-ins.html#keyboard-shortcuts)
  ...
  3. Once your function seems like it will work, immediately run `tar_make()` - I also highly advise making a keyboard shortcut for this.
    - It's also convenient to run `tar_marke(target_name)` to run the pipeline up to, but not past, the target you built, if there are other unrelated targets you don't want to run right now.
...
Hopefully you can see how the plan we have now defeats classes reproducibility issues typically experienced with classic R projects. Explicity, we get a more reproducible workflow since:

  - Our targets are built in a separate session, so cannot be affected the by the state of our interactive development evironment.
...
Imagine you are tasked with peer-reviewing a pipeline. If that pipeline is engineered with `{targets}` your workflow looks like:
  ...
  4. You can context switch away and come back, maybe days later, and do exactly the same thing. All the pipleine's targets are still sitting in the store ready for immediate interactive use.
